...
# =================================================================
# FastNext Orchestrator - Debian 12.9 Lifecycle Manager
# =================================================================
...
cleanup_previous() {
    ...
    if [ -f "$KUBE_CONFIG" ]; then
        ...
        kubectl delete deployment fn-backend fn-frontend --ignore-not-found
...
read -p "Do you want to remove previous versions before installing? (y/n): " confirm
if [[ $confirm == [yY] ]]; then cleanup_previous; fi
...
# 5. GENERATE DIRECTORIES & CODE
mkdir -p "$PROJECT_ROOT/backend" "$PROJECT_ROOT/k8s"
...
@app.get("/api/health")
def health(): return {"status": "Online", "network_browsable": True}
EOF

echo "fastapi==0.109.0" > "$PROJECT_ROOT/backend/requirements.txt"
echo "uvicorn==0.27.0" >> "$PROJECT_ROOT/backend/requirements.txt"
...
echo "Creating Frontend (Next.js)..."
cd "$PROJECT_ROOT"
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-zinc-950 text-white">
      <div className="p-12 bg-zinc-900 border border-blue-500/30 rounded-3xl shadow-2xl text-center">
        <h1 className="text-3xl font-bold text-blue-500 mb-4">FastNext Orchestrator</h1>
        <p className="font-mono text-green-400">API Status: {status}</p>
      </div>
    </main>
  );
}
...
# 7. BUILD, IMPORT, DEPLOY
echo "Building & Syncing..."
